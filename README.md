...
- Allow combined entries in `GameIndex.yaml`. Serial alone is not enough to uniquely identify a game build. So now this system supports `SERIAL_CRC` format as well, and this will take priority over the `SERIAL` format.
